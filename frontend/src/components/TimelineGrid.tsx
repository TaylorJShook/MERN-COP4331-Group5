...
const TaskBlock: React.FC<TaskBlockProps> = React.memo(({ task, position, currentTime, onSelect }) => {
  ...
  const startTime = task.StartDate ? new Date(task.StartDate).toLocaleTimeString([], { 
    hour: 'numeric', 
    minute: '2-digit',
    hour12: true 
  }) : 'No start time';
  const dueTime = task.DueDate ? new Date(task.DueDate).toLocaleTimeString([], { 
    hour: 'numeric', 
    minute: '2-digit',
    hour12: true 
  }) : 'No due time';
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      aria-label={`${task.title || 'Untitled Task'}, ${statusLabel}, from ${startTime} to ${dueTime}`}
...
const TimelineGrid: React.FC<TimelineGridProps> = ({
    ...
    const remeasureTimeout = setTimeout(() => {
      requestAnimationFrame(() => {
        if (firstHourSlotRef.current && timelineTasksRef.current) {
          ...
          const firstSlotTopRelativeToTasks = hourSlotRect.top - tasksContainerRect.top;
  ...
  const hourWindowLocal = (anchorLocal: Date, hour: number): [number, number] => {
  ...
  const { slotHeights, taskPositions } = useMemo(() => {
    ...
    hours.forEach((hour) => {
      ...
      overlappingTasks.forEach((task) => {
        ...
        const overlapMinutes = overlapMinutesInHourLocal(s, e, dayAnchorLocal, hour);
      ...
      columns.forEach((col) => {
        ...
        const ordered = col.slice().sort((a, b) =>
    ...
    const validTasksForPositioning = validTasks.filter((task) => {
      ...
      if (durationDays > 7) {
        console.warn('Task duration exceeds 7 days, may display incorrectly:', task.title);
      }
    ...
    const positions: TaskPosition[] = validTasksForPositioning.map((task) => {
      ...
      if (clampedEndHour < clampedStartHour || (clampedEndHour === clampedStartHour && eMin <= sMin)) {
        return null;
      }
    ...
    columns.forEach((column) => {
      ...
      for (let i = 1; i < column.length; i++) {
        ...
        if (currentTask.top < previousBottom) {
          ...
          const clampedEnd = Math.min(dayEndMs, Math.max(dayStartMs, taskEndMs));
          
          // Use local hours and minutes
          const clampedEndDate = new Date(clampedEnd);
          const visibleEndHoursInt = clampedEndDate.getHours();
          const endMinutes = clampedEndDate.getMinutes() + 
                           clampedEndDate.getSeconds() / 60 + 
                           clampedEndDate.getMilliseconds() / 60000;
          ...
          const endHourSlotHeight = heights.get(visibleEndHoursInt) || hourHeight;
    ...
    positions.forEach((pos) => {
      ...
      const startMinutes = clampedStartDate.getMinutes() + 
                          clampedStartDate.getSeconds() / 60 + 
                          clampedStartDate.getMilliseconds() / 60000;
      ...
      naturalTop += (startMinutes / 60) * (initialHeights.get(visibleStartHoursInt) || hourHeight);
  ...
  const currentTimeIndicator = useMemo(() => {
    ...
    const currentMinutes = now.getMinutes() + now.getSeconds() / 60 + now.getMilliseconds() / 60000;
  ...
  }, [currentTime, slotHeights, hourHeight, baseOffset, selectedDate, startHour, endHour]);
  ...
  useEffect(() => {
    if (selectedTask) {
      const taskExists = tasks.some((task: any) => 
  ...
  const timeSlots: Array<{ hour: number; minute: number; isHalfHour: boolean; height: number }> = [];
  hours.forEach((hour) => {
    timeSlots.push({ 
      hour, 
      minute: 0, 
  ...
  return (
    <div className="timeline-grid" role="region" aria-label="Daily timeline view">
        ...
        <div className="timeline-hours" role="list" aria-label="Hour markers">
          {hours.map((hour) => {
            ...
            return (
              <div 
                key={hour} 
                className={`timeline-hour-marker ${isLastHour ? 'timeline-hour-marker--last' : ''}`}
                style={{ height: `${slotHeight}px` }}
                role="listitem"
              >
                <span className="timeline-hour-label" aria-label={`Hour ${formatHour(hour)}`}>
                  {formatHour(hour)}
                </span>
              </div>
            );
        ...
        <div className="timeline-scroll">
          ...
          <div className="timeline-inner">
            ...
            <div 
              ...
              role="list"
              aria-label={`Timeline tasks for selected day, ${taskPositions.length} task${taskPositions.length !== 1 ? 's' : ''}`}
            >
              {currentTimeIndicator.visible && (
                ...
                >
                  <div className="timeline-current-time__line" aria-hidden="true"></div>
                  <div className="timeline-current-time__dot" aria-hidden="true"></div>
                  <div className="timeline-current-time__label">
                    <span className="timeline-current-time__label-dot" aria-hidden="true"></span>
            ...
            <div className="timeline-sidebar__content">
              ...
              <div className="timeline-sidebar__section">
                ...
                <div className="timeline-sidebar__value timeline-sidebar__status">
                  <span className={`timeline-sidebar__status-badge timeline-sidebar__status-badge--${getTaskStatus(selectedTask, currentTime)}`}>
