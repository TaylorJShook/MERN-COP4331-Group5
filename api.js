...
async function sendVerifyEmail({ to, code, link }) {
  ...
  if (templateId) {
    await sgMail.send({
      ...
      from: FROM,
      templateId,
      dynamic_template_data: { code, link },
    });
  } else {
    await sgMail.send({
      to,
      ...
      html: `<p>Your verification code is <b>${code}</b> (valid 15 minutes).</p><p>Or click to verify: <a href="${link}">${link}</a></p>`,
    });
  }
}
async function sendResetEmail({ to, code, link }) {
  ...
  if (templateId) {
    ...
    await sgMail.send({
      ...
      html: `<p>Your reset code is <b>${code}</b> (valid 15 minutes).</p><p>Or reset via link: <a href="${link}">${link}</a></p>`,
    });
  }
}
...
function decodeJwtUser(jwtToken) {
  ...
  try {
    if (typeof token.decodeToken === "function")
      return token.decodeToken(jwtToken);
  } catch {}
  try {
    ...
    if (authUserId === null || typeof authUserId === "undefined") {
      return {
        ok: false,
          ...
          .status(401)
          .json({ error: "Unable to determine user from token", jwtToken: "" }),
      };
    }
...
exports.setApp = function setApp(app, client) {
    ...
    if (!firstName || !lastName || !login || !password || !email) {
      return res
        ...
        .json({
          ...
          lastName: "",
          jwtToken: "",
          error: "firstName, lastName, login, password, and email are required",
        });
    }
    try {
      ...
      const last = await Users.find({ UserID: { $type: "number" } })
        .project({ UserID: 1 })
        .sort({ UserID: -1 })
        .limit(1)
        .toArray();
      ...
      const link = `${BACKEND_BASE_URL}/api/verify-email-link?login=${encodeURIComponent(
        newUser.Login
      )}&token=${vtoken}`;
      try {
        await sendVerifyEmail({ to: newUser.Email, code, link });
      } catch {}

      const t = token.createToken(
        newUser.FirstName,
        newUser.LastName,
        newUser.UserID ?? ins.insertedId
      );
      return res
        .status(200)
        .json({
          id: newUser.UserID ?? ins.insertedId,
  ...
  app.post("/api/request-email-verification", async (req, res) => {
    ...
    if (!login && !email)
      return res
        .status(400)
        .json({ sent: false, error: "login or email is required" });
    try {
      const u = await db
        .collection("Users")
        .findOne(login ? { Login: login } : { Email: email });
      if (!u)
        return res.status(200).json({ sent: false, error: "user not found" });
      if (!u.Email)
        return res
          .status(200)
          .json({ sent: false, error: "user has no email" });
  ...
  app.post("/api/verify-email", async (req, res) => {
    ...
    if (!login || !code)
      return res
        .status(400)
        .json({ verified: false, error: "login and code are required" });
    try {
      ...
      if (!u.VerificationCode || !u.VerificationExpires)
        return res
          .status(200)
          .json({ verified: false, error: "no code requested" });
      if (new Date() > new Date(u.VerificationExpires))
        return res.status(200).json({ verified: false, error: "code expired" });
      if (String(u.VerificationCode) !== String(code))
        return res.status(200).json({ verified: false, error: "invalid code" });
  ...
  app.get("/api/verify-email-link", async (req, res) => {
    ...
    if (!login || !vtoken)
      return res.status(400).send("Missing login or token");
    try {
      const u = await db.collection("Users").findOne({ Login: login });
      if (!u) return res.status(404).send("User not found");
      if (!u.VerificationToken || !u.VerificationExpires)
        return res.status(400).send("No token issued");
      if (new Date() > new Date(u.VerificationExpires))
        return res.status(400).send("Token expired");
      if (String(u.VerificationToken) !== String(vtoken))
  ...
  app.post("/api/request-password-reset", async (req, res) => {
    ...
    try {
      ...
      const link = `${FRONTEND_BASE_URL}/reset-password?login=${encodeURIComponent(
        u.Login
      )}&token=${rtoken}`;
  ...
  app.post("/api/verify-reset-code", async (req, res) => {
    ...
    if ((!login && !email) || !code)
      return res
        .status(400)
        .json({ verified: false, error: "login or email, code required" });
    try {
      ...
      if (!u)
        return res
          .status(200)
          .json({ verified: false, error: "user not found" });
      if (!u.ResetCode || !u.ResetExpires)
        return res
          .status(200)
          .json({ verified: false, error: "no reset requested" });
      if (new Date() > new Date(u.ResetExpires))
        return res.status(200).json({ verified: false, error: "code expired" });
  ...
  app.post("/api/reset-password-with-code", async (req, res) => {
    ...
    if ((!login && !email) || !code || !newPassword)
      return res
        .status(400)
        .json({ error: "login or email, code, newPassword required" });
    try {
      // Allow either login OR email to find the user
      const query = login ? { Login: login } : { Email: email };
      const u = await db.collection("Users").findOne(query);
      ...
      if (!u.ResetCode || !u.ResetExpires)
        return res.status(200).json({ error: "no reset requested" });
      if (new Date() > new Date(u.ResetExpires))
      ...
      await db
        ...
        .updateOne(
          { _id: u._id },
          {
            ...
            $unset: { ResetCode: "", ResetToken: "", ResetExpires: "" },
          }
        );
  ...
  app.post("/api/reset-password-with-token", async (req, res) => {
    ...
    if ((!login && !email) || !rtoken || !newPassword)
      return res
        .status(400)
        .json({ error: "login or email, token, newPassword required" });
    try {
      // Allow either login OR email to find the user
      const query = login ? { Login: login } : { Email: email };
      const u = await db.collection("Users").findOne(query);
      if (!u) return res.status(200).json({ error: "user not found" });
      if (!u.ResetToken || !u.ResetExpires)
        return res.status(200).json({ error: "no reset requested" });
      if (new Date() > new Date(u.ResetExpires))
        return res.status(200).json({ error: "token expired" });
      if (String(u.ResetToken) !== String(rtoken))
  ...
  app.post("/api/addtodo", async (req, res) => {
    const guard = requireJwt(req, res);
    if (!guard.ok) return guard.end;
    const {
      userId,
      title,
      description,
    ...
    try {
      ...
      return res
        .status(200)
        .json({ id: r.insertedId, error: "", jwtToken: guard.refreshedToken });
    } catch (e) {
      return res
        .status(500)
        .json({ id: null, error: e.toString(), jwtToken: "" });
    }
  });
  ...
  app.post("/api/deletetodo", async (req, res) => {
    ...
    if (!id)
      return res
        .status(400)
        .json({ deletedCount: 0, error: "id is required", jwtToken: "" });
    if (!ObjectId.isValid(id))
      return res
        .status(400)
        .json({ deletedCount: 0, error: "invalid id format", jwtToken: "" });
    try {
      const r = await db
        .collection("Todos")
        .deleteOne({ _id: new ObjectId(id), ...ownerFilter(guard.authUserId) });
      if (r.deletedCount === 0) {
        return res
          .status(403)
          .json({
      ...
      return res
        .status(500)
        .json({ deletedCount: 0, error: e.toString(), jwtToken: "" });
    }
  });
  ...
  app.post("/api/edittodo", async (req, res) => {
    const guard = requireJwt(req, res);
    if (!guard.ok) return guard.end;
    const { id, title, description, startDate, dueDate, priority, completed } =
      req.body;
    if (!id)
      return res
        .status(400)
        .json({ modifiedCount: 0, error: "id is required", jwtToken: "" });
    if (!ObjectId.isValid(id))
      return res
        .status(400)
        .json({ modifiedCount: 0, error: "invalid id format", jwtToken: "" });
    ...
    if (typeof priority !== "undefined")
      $set.Priority = normalizePriority(priority);
    if (!Object.keys($set).length && !Object.keys($unset).length) {
      return res
        .status(400)
        .json({ modifiedCount: 0, error: "no fields to update", jwtToken: "" });
    }
    ...
    try {
      ...
      return res
        .status(500)
        .json({ modifiedCount: 0, error: e.toString(), jwtToken: "" });
    }
  });
  ...
  app.post("/api/gettodos", async (req, res) => {
    ...
    if (!userId)
      return res
        .status(400)
        .json({ results: [], error: "userId is required", jwtToken: "" });
    if (String(userId) !== String(guard.authUserId)) {
      return res
        .status(403)
        .json({
    ...
    try {
      const results = await db
        .collection("Todos")
        .find(ownerFilter(guard.authUserId))
        .sort({ CreatedAt: -1 })
        .toArray();
      ...
      return res
        .status(500)
        .json({ results: [], error: e.toString(), jwtToken: "" });
    }
  });
  ...
  app.post("/api/check-bulk", async (req, res) => {
    ...
    if (!Array.isArray(ids) || typeof completed !== "boolean") {
      return res
        ...
        .json({
          modifiedCount: 0,
          error: "ids[] and completed(boolean) required",
          jwtToken: "",
        });
    }
    ...
    if (!valid.length)
      return res
        .status(400)
        .json({ modifiedCount: 0, error: "no valid ids", jwtToken: "" });
    try {
      const r = await db
        .collection("Todos")
        .updateMany(
  ...
  app.post("/api/next-day", async (req, res) => {
    ...
    try {
      ...
      const newDue = todo.DueDate
        ? addDays(new Date(todo.DueDate), 1)
  ...
  app.post("/api/previous", async (req, res) => {
    ...
    try {
      ...
      const yStart = startOfDay(daysAgo(1)),
        yEnd = endOfDay(daysAgo(1));
      const last7Start = startOfDay(daysAgo(7)),
        last30Start = startOfDay(daysAgo(30));
      ...
      for (const t of tasks) {
        ...
        if (!completedAt) {
          if (includeNoTimestamp) noTimestamp.push(t);
          continue;
        }
        if (completedAt >= todayStart && completedAt <= todayEnd) today.push(t);
        else if (completedAt >= yStart && completedAt <= yEnd)
          yesterday.push(t);
        else if (completedAt > yEnd && completedAt >= last7Start) last7.push(t);
        else if (completedAt < last7Start && completedAt >= last30Start)
          last30.push(t);
        else older.push(t);
      }
